...
function ExecuteModal({ plan, fund, navRows, config, factorResult, hint, error, saving, onClose, onConfirm }) {
  ...
  const canConsiderTactical = factorSnapshot?.actionPriority >= 70
    && factorSnapshot?.dataConfidence?.score >= 70
    && !factorSnapshot?.flags?.includes('NAV_STALE')
    && !factorSnapshot?.flags?.includes('SIGNAL_FUND_MISSING')
    && !factorSnapshot?.flags?.includes('FALLING_KNIFE_RISK');
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"><form className="card grid w-full max-w-lg gap-3 p-5" onSubmit={(e) => { e.preventDefault(); onConfirm(form); }}><h3 className="text-xl font-semibold text-white">执行定投</h3><p className="text-[#bbbbbb]">{plan.fundName} · 计划金额 ¥{Number(plan.amount).toLocaleString('zh-CN')}</p><p className={hint.cls}>{hint.text}</p><div className="rounded-lg bg-[#151515] p-3 text-sm text-[#d4d4d4]"><p>价格状态：{factorSnapshot?.priceCondition.score == null ? '数据不足' : factorSnapshot.priceCondition.score} · 数据置信度：{factorSnapshot?.dataConfidence.score ?? '数据不足'}</p><p className="mt-1 text-[#888888]">行动说明：{factorSnapshot?.signalFundCode ? factorSnapshot.explanation : '数据不足：尚未配置该类别信号基金'}</p><p className="mt-1 text-[#888888]">风险标记：{factorSnapshot?.flags?.length ? factorSnapshot.flags.join('、') : '暂无'}</p><p className="mt-1 text-[#888888]">默认金额保持计划金额；不自动改金额。{canConsiderTactical ? '可考虑使用额外战术资金（本次不自动执行）' : '暂无额外战术资金提示。'}</p></div>{error && <p className="danger">{error}</p>}<input className="input" type="number" step="0.01" placeholder="计划金额" value={form.totalAmount} onChange={e => set('totalAmount', e.target.value)} /><label className="grid gap-1"><input className="input" type="number" step="0.01" min="0" placeholder="手续费" value={form.fee} onChange={e => set('fee', e.target.value)} /><span className="text-xs text-[#888888]">费率 {(feeRate * 100).toFixed(2)}%</span></label><input className="input" type="number" step="0.0001" placeholder="确认净值" value={form.price} onChange={e => set('price', e.target.value)} /><input className="input" type="number" step="0.0001" placeholder="确认份额" value={form.shares} onChange={e => set('shares', e.target.value)} /><div className="flex justify-end gap-2"><button type="button" className="btn2" onClick={onClose}>取消</button><button className="btn" disabled={saving}>{saving ? '执行中...' : '确认执行'}</button></div></form></div>;
}
