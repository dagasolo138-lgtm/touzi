...
function ExecuteModal({ plan, fund, navRows, config, hint, error, saving, onClose, onConfirm }) {
  ...
  const factorSnapshot = fund && config ? buildFactorSnapshot({ category: fund.category, signalFundCode: plan.fundCode, navRows: navRows.filter((r) => r.fundCode === plan.fundCode), actualWeight: 0, targetWeight: 0, asOfDate: new Date().toISOString().slice(0, 10), settings: config.factorSettings }) : null;
