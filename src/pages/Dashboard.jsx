...
export default function Dashboard(){
  ...
  useEffect(()=>{loadAll(); getDcaPlans().then((plans)=>setDuePlans(plans.filter((plan)=>isDueToday(plan,today())))); Promise.all([getSnapshots(),getTransactions()]).then(([s,t])=>{setSnapshots(s); setTransactions(t);}).catch(()=>{setSnapshots([]); setTransactions([]);});},[loadAll]);
  ...
  const cards=[['总市值',formatMoney(summary.totalValue),''],['账面盈亏',`${formatMoney(summary.totalPnl)} (${formatPct(summary.pnlPct)})`,summary.totalPnl>=0?'good':'danger'],['累计TWR',twrSummary.performanceMethod==='twr'?formatPct(twrSummary.cumulativeReturn):'数据不足',(twrSummary.cumulativeReturn||0)>=0?'good':'danger'],['组合更新时间',updated?new Date(updated).toLocaleString('zh-CN'):'暂无','']];
