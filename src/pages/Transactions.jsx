...
export default function Transactions() {
  ...
  function validateTransaction(tx, currentEditing) {
    if (tx.type !== 'sell') return;
    const holding = summary.holdings.find((h) => h.code === tx.fundCode);
    const editingShares = currentEditing?.type === 'sell' && currentEditing.fundCode === tx.fundCode ? Number(currentEditing.shares) || 0 : 0;
    const currentShares = (Number(holding?.shares) || 0) + editingShares;
    const sellShares = Number(tx.shares) || 0;

    if (currentShares + 1e-8 < sellShares) {
      throw new Error(`卖出份额（${sellShares}）超过当前持有份额（${currentShares}），请检查后重试。`);
    }
  }
