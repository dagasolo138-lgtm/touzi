...
export async function exportData() { return { funds: await all('funds'), transactions: await all('transactions'), navHistory: await all('navHistory'), snapshots: await all('snapshots'), aiLogs: await all('aiLogs') }; }
export async function importData(data) { const db = await dbPromise; const tx = db.transaction(['funds', 'transactions', 'navHistory', 'snapshots', 'aiLogs'], 'readwrite'); for (const store of tx.objectStoreNames) for (const item of data[store] || []) await tx.objectStore(store).put(item); await tx.done; }
export async function clearAllData() { const db = await dbPromise; const tx = db.transaction(['funds', 'transactions', 'navHistory', 'snapshots', 'aiLogs'], 'readwrite'); for (const s of tx.objectStoreNames) await tx.objectStore(s).clear(); await tx.done; }
