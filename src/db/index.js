...
export async function clearAllData() { const db = await dbPromise; const tx = db.transaction(['funds', 'transactions', 'navHistory', 'snapshots', 'aiLogs', 'conversations', 'dcaPlans'], 'readwrite'); for (const s of tx.objectStoreNames) await tx.objectStore(s).clear(); await tx.done; }
