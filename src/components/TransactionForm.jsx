...
export default function TransactionForm({ funds = [], editing, onSaved, onBeforeSave }) {
  ...
  useEffect(() => {
    if (editing) setForm({ ...editing, amount: (editing.amountCents || 0) / 100, fee: (editing.feeCents || 0) / 100 });
    else if (funds[0]) setForm((f) => ({ ...f, fundCode: f.fundCode || funds[0].code }));
  }, [editing, funds]);

  const set = (k, v) => setForm((f) => ({
    ...f,
    [k]: v,
    ...(k === 'shares' || k === 'price' ? { amount: ((k === 'shares' ? v : f.shares) * (k === 'price' ? v : f.price) || 0).toFixed(2) } : {}),
  }));
