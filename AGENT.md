...
- IndexedDB 升级到版本3，新增 `dcaPlans` 表，导入导出与清空数据流程覆盖该表。
- `src/pages/DCA.jsx` 实现定投计划列表、新建/编辑、暂停/启用、删除、手动执行、到期计算、RSI提示和策略对比图。
- `Dashboard.jsx` 复用 `DCA.jsx` 导出的 `isDueToday` 展示今日定投提醒。
